...
/* 長所と短所。**5因子それぞれの段から組み立てる。**

   4文字から作ってはいけない。4文字だと、誠実性が10の人も6の人も同じ J に
   なって同じ文が出る。実際それで ESFJ の短所に「先送りする」と書き、誠実性が
   満点の人に出て矛盾した。段から作れば、五角形の数字とも、その下の因子ごとの
   読みとも、必ず一致する。

   [段0, 段1, 段2, 段3] の順で、それぞれ [長所, 短所]。
   **どの段にも長所と短所を1つずつ置くこと。** 低い側を短所だけにすると、
   全員が高い側に寄って測定が歪む。 */
const TRAIT = {
  e: [['一人で深く集中できる',       '人前では存在感が出にくい'],
      ['少人数でこそ深い話ができる', '大人数の場で消耗しやすい'],
      ['場に合わせて動ける',         '一人の時間を取り損ねやすい'],
      ['場を開いて人を集められる',   '喋りすぎて相手の番を奪いやすい']],
  a: [['言うべきことを言い切れる',   '言い方で角が立つ'],
      ['必要なら遠慮なく言える',     '配慮の言葉が足りなくなる'],
      ['相手を見て言葉を選べる',     '持ち帰って悩みすぎる'],
      ['場を壊さずまとめられる',     '言いにくいことを飲み込む']],
  c: [['その場の機転で立て直せる',   '締切と持ち物が抜ける'],
      ['力の抜きどころを知っている', '放っておくとゆるむ'],
      ['決めたことは守る',           '無理な頼みまで抱えやすい'],
      ['段取りと期限が確実',         '他人のゆるさに苛立つ']],
  s: [['人の不調に最初に気づく',     '小さなことで消耗する'],
      ['人の気持ちを想像できる',     '気になることを引きずる'],
      ['慌ただしい場面でも判断を保てる', '他人事に見られることがある'],
      ['危機でも崩れない',           '周りの動揺に気づかない']],
  o: [['判断がぶれない',             '新しいやり方を受け入れにくい'],
      ['大きく外さない選び方をする', '面白がる幅が狭い'],
      ['知らないことをひとまず試せる', '関心が広がりすぎる'],
      ['新しい風を持ち込める',       '飽きが早くやりかけが積む']]
};

/* 10段階の表示値。/* 10段階の表示値。0〜7 を 1〜10 に写す。
...
/* 五角形の下に置く、長所と短所。際立っている因子から並べるので、
   その人でいちばん効いているものが先に来る */
const traitBlock = s => {
  const fs = standout(s);
  const pick = i => fs.map(f => TRAIT[f.id][s[f.id].band][i]).join('、') + '。';
  return `<h4>${esc(archName(s))}（${codeOf(s)}）の主な特徴</h4>
    <dl><dt>長所</dt><dd>${pick(0)}</dd>
        <dt>短所</dt><dd>${pick(1)}</dd></dl>`;
};
