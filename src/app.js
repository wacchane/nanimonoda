...
const MODES = [
  ...
  { id: 'peer', kind: 'peer', qk: 'other', name: '他人を診断する',  hint: '結果をリンクで相手に送れる' }
];
...
/* ===== 結果の言葉 =====
   **口調は言い切りで統一する。** デスマス調と混ぜないこと。
   辛口とエンタメ性に合うのは言い切りのほうで、混在すると診断の芯がぼやける。

   **書き方の約束: ちょっと辛口に入って、必ず最後は認めて終わる。**
   低い側を欠点として言い切らない。どの因子も「高い側が良い」という意味ではない。 */

/* 名前の芯。最も際立つ因子から取る（低い側 / 高い側）。
   **硬い役職名にしないこと。** 覚えて人に言いたくなる名前にする */
const ARCHETYPE = {
  e: ['省エネ賢者', '宴の主役'],
  a: ['斬り込み隊長', '和ませ上手'],
  c: ['アドリブ職人', '段取りの鬼'],
...
/* 名前の冠。2番目に際立つ因子から取る。20語で100通りになる */
const PREFIX = {
  e: ['物静かな', 'やたら喋る'],
  a: ['歯に衣着せぬ', 'やさしすぎる'],
  c: ['ゆるふわ', '抜かりない'],
...
const READ = {
  e: ['一人の時間がないと回復しない。誘いを断ってばかりに見えるが、少人数の場でこそ深い話ができる。',
      'にぎやかな場も嫌ではないのに、長引くと急に無口になる。それでも自分のペースを崩さないぶん、無理がない。',
      '人と関わると元気が出るほう。前に出すぎないので物足りなく見えることもあるが、場に合わせて動ける。',
      '人がいるほど調子が上がる。喋りすぎて後で反省しがちだが、あなたがいないと始まらない場が確実にある。'],
  a: ['思ったことをそのまま言うので、たまに空気が凍る。ただ判断は速く、その率直さを信頼している人は少なくない。',
      '必要なら遠慮なく言える。言い方が足りない場面もあるが、相手を見て加減する分別もある。',
      '相手の気持ちを考えて言葉を選べる。持ち帰って悩みすぎるきらいはあるが、そのおかげで守られた関係がある。',
      '波風を立てないことを優先する。言うべきことを飲み込みがちだが、あなたの周りが穏やかなのは偶然ではない。'],
  c: ['締切も持ち物も、だいたいで進む。だらしないと言われることもあるが、崩れた場を機転で立て直す力は本物。',
      'やるべき場面では合わせられるが、放っておくとゆるむ。裏を返せば、力の抜きどころを知っている。',
      '決めたことは守り、無理な約束もしない。派手さはないが、この安定感が周りの前提になっている。',
      '段取りも期限も確実。他人のゆるさに苛立ちがちだが、任せれば必ず終わるという信頼はここから来ている。'],
  s: ['出来事に強く反応し、後から一人で反芻する。消耗は大きいが、人の不調に最初に気づけるのはこの感度のおかげ。',
      '気になることは引きずるが、翌日には戻ってくる。この揺れがあるから、人の気持ちを想像できる。',
      'たいていのことでは動じない。冷たく見えることもあるが、慌ただしい場面ほど頼りになる。',
      '何が起きても平静。周りの動揺に気づかないことがある一方、本当にまずい場面で崩れないのは大きな強み。'],
  o: ['慣れたやり方を守り、実際に役立つかで判断する。頭が固いと言われることもあるが、その一貫性が判断のぶれなさになっている。',
      '新しいものにも触れるが、基本は確実なほうを選ぶ。地味だが、大きく外さない選び方。',
      '知らないことに興味を持ち、ひとまず試す。全部は追いかけない加減の良さがある。',
      '新しいものへ次々に向かう。飽きるのも早く、やりかけが積み上がりがちだが、新しい風を入れられるのはあなた。']
};
...
const ROLE_NOTE = 'どの因子も「高い側が良い」という意味ではない。同じ性質が、ある場面では強みに、別の場面では弱みになる。これは傾向であって、能力や成果を測ったものでもない。';
const PEER_NOTE = '他己評価は、外から見える行動しか拾えない。一人の時間にホッとするか、人と会った後に疲れるかといった内側の項目は、相手には推測でしか答えられない。そのぶんは差し引いて見たほうがいい。';
...
function renderHome() {
  ...
  cb.textContent = ready
    ...
    : (d.self ? '他己評価が届くと比べられる' : 'まず自分を診断する');
}
...
function peerLink() {
  ...
  if (!nick) { toast('ニックネームを入れて'); $('p-nick').focus(); return null; }
...
function gapCard(me, agg, n) {
  ...
  if (abs < 15) {
    verdict = '見え方はほぼ一致';
    desc = `自分の見立てと、${who}から見えているあなたがほぼ重なっている。自己像と外からの印象がずれていない。`;
  } else if (abs < 35) {
    verdict = '少しずれている';
    desc = open
      ? `${who}からは、自分で思っているより外に開いて見えている。自己評価が控えめなだけかもしれないし、場に合わせて振る舞えているぶん内側の消耗が見えていないだけかもしれない。`
      : `${who}からは、自分で思っているより内に寄って見えている。出しているつもりの部分が、まだ届いていない。`;
  } else {
    verdict = '大きくずれている';
    desc = open
      ? `${who}から見たあなたは、自分の見立てよりかなり外向き。周囲の期待と、自分が実際に払っている労力の差が開きやすい。`
      : `${who}から見たあなたは、自分の見立てよりかなり内向き。自分では出しているつもりのものが、伝わっていない。`;
  }
...
$('btn-psend').onclick = async () => {
  ...
  await shareOut({
    title: '何者か',
    text: `${l.nick}が見たあなたを診断した。開くと自分の結果と並べて比べられる。`,
    url: l.url
  }, 'リンクをコピーしました');
};
$('btn-pcopy').onclick = () => {
...
function boot() {
  ...
  if (got) {
    ...
    renderHome();
    showInbox(got, idx);
    if (full) toast(`他己評価は${PEER_MAX}件まで。古いものから外した`);
    return;
  }
  renderHome();
}
